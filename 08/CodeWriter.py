...
class CodeWriter:
    ...
    def write_Init(self) -> None:
        "Write the initial booting code"
        bootstrap_code = "// Bootstrap code \n @256\n D=A\n @SP\n M=D\n"     
        bootstrap_code = bootstrap_code.replace(" ","")
        self.__output_stream.write(bootstrap_code)
        self.write_call("Sys.init",0)   

    def set_current_func(self, func_name: str) -> None:
        """ Sets the current function and resets the current function counter
        """
    ...
    def write_function(self, function_name: str, n_vars: int) -> None:
        ...
        output_cmd = "(" + function_name + ")\n"
        for i in range(n_vars):
            output_cmd += "@0 \n D=A \n @SP \n A=M \n M=D \n @SP \n M=M+1 \n"
        output_cmd = output_cmd.replace(" ","")
        self.__output_stream.write(output_cmd)
    ...
    def write_call(self, function_name: str, n_args: int) -> None:
        ...
        num = str (5 + n_args)
        cur_index = str(self.__func_counter)
        return_addr = self.__current_func +"$ret." + cur_index
        push_cmd = "D=M \n @SP \n A=M \n M=D \n @SP \n M=M+1 \n"
        output_cmd = "@" + return_addr +"\n D=A \n @SP \n A=M \n M=D \n @SP \n M=M+1 \n" #Push address
        output_cmd += "@LCL\n" + push_cmd + "@ARG\n" + push_cmd + "@THIS\n" + push_cmd + "@THAT\n" + push_cmd #Push memory segments
